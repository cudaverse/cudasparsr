...
Row and column names remain attached to the sparse object and are restored by
`to_dgCMatrix()`. Sparse-dense multiplication verifies labels on the
contracted dimension when both inputs provide them, then carries sparse row
names and dense output column names into the result.
