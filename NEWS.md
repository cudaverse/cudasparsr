...
- Sparse row and column names are now retained across construction, format
  conversion, CPU/CUDA storage, `dgCMatrix` round-trips, products, and
  reductions. Labeled sparse-dense products reject incompatible contracted
  dimensions.
...
- `drop_zeros` now requires one non-missing logical value instead of silently
  treating invalid values as `FALSE`.
- Declared `cudatensr (>= 0.1.1)` as the tested dense-core compatibility floor.
